...
public class Cone implements Serializable {
     ...
     *            - the base radius
     */
    public Cone(final int hight, final int radius) throws ConeException {
        // checking for exceptions
        if (hight <= 0) {
            throw new ConeException("Hight can't be <=0", hight);
        }
        if (radius <= 0) {
            throw new ConeException("Radius can't be <=0", hight);
        }
        this.hight = hight;
        this.radius = radius;
    }
